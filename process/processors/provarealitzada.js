...
var updateBlock = function(query, blockIndex, callback) {
    ...
    //collection.find(query).sort({ "actor.account.name": 1 }).limit(pageNumber).skip(skip).forEach(function(err, doc) {
        ...
        if (doc != null) {
            ...
            var qualification = doc.object.definition.extensions['edu:uoc:la:subject'].evaluation.validationTest.qualification;
...
var query = {
    $and: [{
        ...
        "object.definition.extensions.edu:uoc:la:subject.evaluation.validationTest.qualification": {
            $not: /^N$/
        }
    }
]};
